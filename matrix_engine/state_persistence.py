...
Boot recovery:
  restore_state() reads the latest row per symbol×timeframe and injects into
  the indicator ring buffers and strategy state machines via the provided
  callbacks.

No time.sleep. All I/O via asyncio.to_thread (non-blocking SQLite writes).
"""
...
class StatePersistence:
    ...
    Usage:
        ...
        await persist.initialise()          # Creates tables, boots indexes
        asyncio.create_task(persist.run())  # Start snapshot loop
        ...
        snapshots = persist.restore_state() # Called on reboot
    """
    ...
    def __init__(
        ...
        self._running = False
        # Callbacks registered externally so we avoid circular imports
    ...
    def register_strategy_b_provider(self, fn: Callable) -> None:
        """fn() -> Dict[str, _TrapState-like dict]"""
        self._strategy_b_provider = fn

    def register_risk_provider(self, fn: Callable) -> None:
        """fn() -> List[dict] with keys: client_id, capital, max_risk_pct, daily_pnl, ..."""
    ...
    async def _on_candle_close(self, candle: CandleEvent) -> None:
        ...
        if self._strategy_b_provider and candle.timeframe == self._cfg.candle_timeframes[-1]:
            ...
            for underlying, st in states.items():
                rows_strat.append({
                    ...
                    "void_since": st.get("void_since", None),
                    "saved_at": now_str,
                })

        # Risk params (on any candle close — cheap dict snapshot)
        ...
        # Write to SQLite via thread pool so event loop stays unblocked
    ...
    def restore_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Latest snapshot per symbol × timeframe
            cur.execute("""
                SELECT cs.*
            ...
            # All open order tickets
            cur.execute("SELECT * FROM order_tickets WHERE status = 'open' ORDER BY saved_at DESC")
    ...
    def persist_order_ticket(
        ...
        """Called synchronously by execution layer after a fill is confirmed."""
        row = {
            "client_id": client_id,
        ...
        try:
            ...
            con.execute("""
                INSERT INTO order_tickets
                    (client_id, broker_symbol, side, qty, order_id, avg_price, strategy_tag, status, saved_at)
                VALUES
                    (:client_id, :broker_symbol, :side, :qty, :order_id, :avg_price, :strategy_tag, :status, :saved_at)
            """, row)
            con.commit()
            ...
            logger.error("StatePersistence: persist_order_ticket() error: %s", exc)

    def close_order_ticket(self, order_id: str) -> None:
        """Mark an order ticket as closed when the position exits."""
        try:
            con = sqlite3.connect(self._db_path)
            con.execute(
            ...
            logger.error("StatePersistence: close_order_ticket() error: %s", exc)
            ...
            if rows_strat:
                con.executemany("""
                    INSERT INTO strategy_b_state
                        (underlying, phase, rolling_base, htf_entry_level, trap_level, trap_type, void_since, saved_at)
                    VALUES
                        (:underlying, :phase, :rolling_base, :htf_entry_level, :trap_level, :trap_type, :void_since, :saved_at)
                """, rows_strat)
            if rows_risk:
                con.executemany("""
                    INSERT OR REPLACE INTO risk_params
                        (client_id, capital, max_risk_pct, daily_pnl, trade_count, is_halted, saved_at)
                    VALUES
                        (:client_id, :capital, :max_risk_pct, :daily_pnl, :trade_count, :is_halted, :saved_at)
                """, rows_risk)
            con.commit()
