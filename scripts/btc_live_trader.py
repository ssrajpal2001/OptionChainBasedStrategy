...
def detect_signal(df_1m: pd.DataFrame, htf_min: int, sub_min: int) -> Optional[dict]:
    ...
    for htf_z in htf_zones:
        ...
        return {
            ...
            "trigger"  : trigger,
            "t1"       : t1,
            "sl_init"  : _init_sl(htf_z, 0),   # zone edge (sl_buf added at entry)
        }
    return None
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                ...
                # Check if current price is at the trigger level
                trig = signal["trigger"]
                is_long = signal["is_long"]
                in_range = (price <= trig * 1.001) if is_long else (price >= trig * 0.999)

                if in_range:
                    log.info("Signal: %s zone=%s trigger=%.0f t1=%.0f",
                             "LONG" if is_long else "SHORT",
                             signal["zone_key"], trig, signal["t1"])
                    self._enter(signal, price)
                else:
                    log.info("Signal zone=%s trigger=%.0f but price=%.0f not at trigger yet",
                             signal["zone_key"], trig, price)
